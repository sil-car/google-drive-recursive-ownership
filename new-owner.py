...
def get_drive_service():
    ...
    print(f'Use this link for authorization: {authorize_url}')
...
def grant_ownership(service, drive_item, prefix, permission_id, show_already_owned):
    ...
    for owner in drive_item['owners']:
        if owner['permissionId'] == permission_id:
            if show_already_owned:
                print(f'Item "{full_path}" already has the right owner.')
            return
        elif owner['isAuthenticatedUser']:
            current_user_owns = True

    print(f'Item "{full_path}" needs ownership granted.')

    if not current_user_owns:
        print('    But, current user does not own the item.')
        return
    ...
    try:
        ...
        if e.resp.status != 404:
            print(f'An error occurred updating ownership permissions: {e}')
            return
